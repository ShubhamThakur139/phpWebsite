...
while($row = $result->fetch_assoc()){
    if(isset($_REQUEST['checkID']) && $_REQUEST['checkID'] == $row['course_id']){
        ...
        if(($row['course_id']) == ($_REQUEST['checkID'])){
            ...
            ?>
            <h3 class="bg-dark text-white p-2 mt-4">Course ID: <?php if(isset($row['course_id'])) { echo $_SESSION['course_id']; } ?>
            Course Name: <?php if(isset($row['course_name'])) { echo $_SESSION['course_name']; } ?></h3>
            <?php
